...
export async function POST(request: Request) {
  ...
  try{   
    ...
    const validationResult = usersValidator({ userName, email, phoneNumber, password })   
    if(validationResult !== true){      
      ...
      return Response.json({ message: "User register Successfully :))" },{status:201 , headers:{"Set-Cookie":`token=${accessToken};path=/;httpOnly=true`}});
    }catch (err) {
